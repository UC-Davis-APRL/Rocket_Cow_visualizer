...
def pypropep_to_dataframe(p, ox, fuel):
    Parameters = ['of (wt ratio)', 'p (psi)', 't (K)', 'rho (kg/m^3)', 'v (m/s)', 'Isp (s)', 'Ivac (m/s)', 'c* (m/s)',
                  'cf', 'sound (m/s)', 'A/At', 'cp (kJ/kg-K)', 'cv (kJ/kg-K)', 'gamma', 'mol mass (g/mol)',
    ...
    for i, c in enumerate(positions):
        ...
        df.loc[c, 'rho (kg/m^3)'] = (p._equil_objs[i].properties.P * 101325 * p._equil_objs[i].properties.M / 1000) / (
                    p._equil_objs[
                        i].properties.T * 8.314)  # rho (kg/m^3) = (P (atm) * 101325 (Pa) / 1 (atm) * M (g/mol) * 1 kg/1000 g)/( T (K) * R (m^3-Pa/mol-k))
...
def gen_plot(df, pos, var, plot_type='heatmap', ):
    ...
    if plot_type == 'heatmap':
        fig = px.imshow(data_filtered, width=600, height=600, origin='lower', color_continuous_scale='viridis',
                        ...
                        title="{} of {} & {} Engine at the {}".format(var, df.attrs['ox'][1], df.attrs['fuel'][1], pos),
                        aspect="auto")
    elif plot_type == 'surface':
        surface = go.Surface(z=data_filtered.values, x=data_filtered.columns.values, y=data_filtered.index.values,
                             colorscale='Viridis')
...
with st.form('OF & Pressure form'):
    ...
    col2.selectbox('Oxidizer:', ['OXYGEN (LIQUID)', 'OXYGEN (GAS)', 'NITROUS OXIDE', 'AIR (DRY AT SEA LEVEL)'],
                   key='ox')
...
if state.run_button:
    ...
    with col2:
        st.selectbox('Variable:', ['t (K)', 'rho (kg/m^3)', 'v (m/s)', 'Isp (s)', 'Ivac (m/s)', 'c* (m/s)',
                                   'cf', 'sound (m/s)', 'A/At', 'cp (kJ/kg-K)', 'cv (kJ/kg-K)', 'gamma',
                                   'mol mass (g/mol)', 'h (kJ/kg)',
    ...
    if state.plot_button:
        state.figure, state.data_filtered = gen_plot(state.data, state.plot_pos, state.plot_var,
